...
def main(key_byte=0, n_traces=10, n_samples=2000, threshold=0.5):
    ...
    for i in range(n_traces):
        plt.plot(traces[i] + i*3, alpha=0.7, label=f'Trace {i}')
    plt.title('Original Traces (offset for visibility)')
    plt.xlabel('Sample')
    plt.show()
    ...
    plt.figure(figsize=(15, 4))
    for i in range(n_traces):
        plt.plot(aligned_traces[i] + i*3, alpha=0.7, label=f'Aligned Trace {i}')
    plt.title('Aligned Traces (offset for visibility)')
    plt.xlabel('Sample')
    plt.show()
    
    # Plot overlayed for comparison
    plt.figure(figsize=(15, 4))
    for i in range(n_traces):
        plt.plot(aligned_traces[i], alpha=0.5)
    plt.title('Aligned Traces Overlayed')
    plt.xlabel('Sample')
    plt.show()
